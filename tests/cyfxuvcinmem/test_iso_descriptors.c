/*
 * UVC 1.5 Isochronous Implementation Unit Tests
 * ============================================
 *
 * Unit tests specific to the cyfxuvcinmem (isochronous) implementation.
 * Tests UVC 1.5 descriptor compliance and isochronous-specific features.
 */

#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
#include <string.h>
#include <assert.h>

// Include UVC headers for isochronous implementation
#include "../../cyfxuvcinmem/cyfxuvcinmem.h"

// Test framework macros
#define TEST_ASSERT(condition, message) \
    do { \
        if (!(condition)) { \
            printf("FAIL: %s - %s\n", __func__, message); \
            return 0; \
        } \
    } while(0)

#define TEST_PASS() \
    do { \
        printf("PASS: %s\n", __func__); \
        return 1; \
    } while(0)

// External descriptor declarations for isochronous implementation
extern const uint8_t CyFxUSBSSConfigDscr[];
extern const uint8_t CyFxUSBHSConfigDscr[];

// Test counter
static int tests_passed = 0;
static int tests_total = 0;

// Helper function to run a test
#define RUN_TEST(test_func) \
    do { \
        tests_total++; \
        if (test_func()) tests_passed++; \
    } while(0)

/**
 * Test UVC 1.5 version in isochronous descriptors
 */
int test_iso_uvc_version()
{
    const uint8_t *ss_desc = CyFxUSBSSConfigDscr;
    const uint8_t *hs_desc = CyFxUSBHSConfigDscr;

    // Find VC interface header descriptor in SS config
    const uint8_t *vc_header_ss = &ss_desc[9 + 8 + 9];

    // Verify UVC 1.5 version (bytes 3-4): should be 0x0150
    TEST_ASSERT(vc_header_ss[3] == 0x50 && vc_header_ss[4] == 0x01,
                "SS isochronous descriptor should indicate UVC 1.5 (0x0150)");

    // Same test for HS descriptor
    const uint8_t *vc_header_hs = &hs_desc[9 + 8 + 9];
    TEST_ASSERT(vc_header_hs[3] == 0x50 && vc_header_hs[4] == 0x01,
                "HS isochronous descriptor should indicate UVC 1.5 (0x0150)");

    TEST_PASS();
}

/**
 * Test isochronous endpoint descriptor
 */
int test_iso_endpoint_descriptor()
{
    const uint8_t *ss_desc = CyFxUSBSSConfigDscr;

    // Search for isochronous endpoint descriptor
    int desc_offset = 9;
    int found_iso_endpoint = 0;

    while (desc_offset < 0xE0) { // Total descriptor length for SS
        uint8_t desc_len = ss_desc[desc_offset];
        if (desc_len == 0) break;

        uint8_t desc_type = ss_desc[desc_offset + 1];

        // Look for endpoint descriptor (type 0x05)
        if (desc_type == 0x05) {
            uint8_t ep_address = ss_desc[desc_offset + 2];
            uint8_t ep_attributes = ss_desc[desc_offset + 3];

            // Check if this is the isochronous video endpoint (0x83)
            if (ep_address == 0x83 && (ep_attributes & 0x03) == 0x01) { // Isochronous
                found_iso_endpoint = 1;

                // Verify isochronous endpoint attributes
                TEST_ASSERT((ep_attributes & 0x0C) == 0x00,
                            "Isochronous endpoint should have no synchronization");

                uint16_t max_packet_size = ss_desc[desc_offset + 4] |
                                         (ss_desc[desc_offset + 5] << 8);
                TEST_ASSERT(max_packet_size == 1024,
                            "Isochronous endpoint max packet size should be 1024 bytes");

                break;
            }
        }

        desc_offset += desc_len;
    }

    TEST_ASSERT(found_iso_endpoint, "Isochronous video endpoint (0x83) should be present");
    TEST_PASS();
}

/**
 * Test SuperSpeed endpoint companion descriptor for isochronous
 */
int test_iso_ss_endpoint_companion()
{
    const uint8_t *ss_desc = CyFxUSBSSConfigDscr;

    // Search for SuperSpeed endpoint companion descriptor
    int desc_offset = 9;
    int found_companion = 0;

    while (desc_offset < 0xE0) {
        uint8_t desc_len = ss_desc[desc_offset];
        if (desc_len == 0) break;

        uint8_t desc_type = ss_desc[desc_offset + 1];

        // Look for SS endpoint companion descriptor (type 0x30)
        if (desc_type == 0x30 && desc_len == 0x06) {
            found_companion = 1;

            uint8_t max_burst = ss_desc[desc_offset + 2];
            uint8_t attributes = ss_desc[desc_offset + 3];
            uint16_t bytes_per_interval = ss_desc[desc_offset + 4] |
                                        (ss_desc[desc_offset + 5] << 8);

            // Verify SS companion descriptor for isochronous
            TEST_ASSERT(max_burst == 0x00, "SS companion max burst should be 0 for isochronous");
            TEST_ASSERT((attributes & 0x03) == 0x02, "SS companion should indicate 3 packets per microframe");
            TEST_ASSERT(bytes_per_interval == 3072, "Bytes per interval should be 3072 (3 * 1024)");

            break;
        }

        desc_offset += desc_len;
    }

    TEST_ASSERT(found_companion, "SuperSpeed endpoint companion descriptor should be present");
    TEST_PASS();
}

/**
 * Test isochronous-specific descriptor sizes
 */
int test_iso_descriptor_sizes()
{
    const uint8_t *ss_desc = CyFxUSBSSConfigDscr;
    const uint8_t *hs_desc = CyFxUSBHSConfigDscr;

    // Check total configuration descriptor length
    uint16_t ss_total_len = ss_desc[2] | (ss_desc[3] << 8);
    uint16_t hs_total_len = hs_desc[2] | (hs_desc[3] << 8);

    TEST_ASSERT(ss_total_len == 0x00E0, "SS isochronous config total length should be 0xE0 (224 bytes)");
    TEST_ASSERT(hs_total_len == 0x00D4, "HS isochronous config total length should be 0xD4 (212 bytes)");

    // Check VC interface total class descriptor size
    const uint8_t *vc_header_ss = &ss_desc[9 + 8 + 9];
    uint16_t vc_class_total_ss = vc_header_ss[5] | (vc_header_ss[6] << 8);

    TEST_ASSERT(vc_class_total_ss == 0x005E, "SS VC class descriptors total should be 0x5E (94 bytes)");

    TEST_PASS();
}

/**
 * Test encoding unit in isochronous implementation
 */
int test_iso_encoding_unit()
{
    const uint8_t *ss_desc = CyFxUSBSSConfigDscr;

    // Search for encoding unit descriptor (subtype 0x07)
    int desc_offset = 9;
    int found_encoding_unit = 0;

    while (desc_offset < 0xE0) {
        uint8_t desc_len = ss_desc[desc_offset];
        if (desc_len == 0) break;

        uint8_t desc_type = ss_desc[desc_offset + 1];
        uint8_t desc_subtype = ss_desc[desc_offset + 2];

        // Look for class-specific VC interface descriptor with encoding unit subtype
        if (desc_type == 0x24 && desc_subtype == CY_FX_UVC_VC_ENCODING_UNIT) {
            found_encoding_unit = 1;

            // Verify encoding unit structure for isochronous implementation
            TEST_ASSERT(desc_len == 0x0D, "Encoding unit descriptor should be 13 bytes");

            uint8_t unit_id = ss_desc[desc_offset + 3];
            uint8_t source_id = ss_desc[desc_offset + 4];

            TEST_ASSERT(unit_id == 0x05, "Encoding unit ID should be 5 in isochronous implementation");
            TEST_ASSERT(source_id == 0x03, "Encoding unit source should be extension unit (ID 3)");

            break;
        }

        desc_offset += desc_len;
    }

    TEST_ASSERT(found_encoding_unit, "Encoding unit descriptor should be present in isochronous implementation");
    TEST_PASS();
}

/**
 * Test UVC 1.5 constants specific to isochronous implementation
 */
int test_iso_uvc15_constants()
{
    // Test H.264 format constants
    TEST_ASSERT(CY_FX_UVC_VS_FORMAT_H264 == 0x10, "H.264 format subtype should be 0x10");
    TEST_ASSERT(CY_FX_UVC_VS_FORMAT_H264_SIMULCAST == 0x11, "H.264 simulcast format subtype should be 0x11");
    TEST_ASSERT(CY_FX_UVC_VS_FORMAT_H265 == 0x12, "H.265 format subtype should be 0x12");

    // Test frame descriptor constants
    TEST_ASSERT(CY_FX_UVC_VS_FRAME_H264 == 0x11, "H.264 frame subtype should be 0x11");
    TEST_ASSERT(CY_FX_UVC_VS_FRAME_H265 == 0x13, "H.265 frame subtype should be 0x13");

    // Test encoding unit constant
    TEST_ASSERT(CY_FX_UVC_VC_ENCODING_UNIT == 0x07, "Encoding unit subtype should be 0x07");

    // Test some control selectors
    TEST_ASSERT(CY_FX_UVC_EU_SELECT_LAYER_CONTROL == 0x01, "Select Layer Control should be 0x01");
    TEST_ASSERT(CY_FX_UVC_EU_RATE_CONTROL_MODE_CONTROL == 0x06, "Rate Control Mode should be 0x06");
    TEST_ASSERT(CY_FX_UVC_EU_AVERAGE_BITRATE_CONTROL == 0x07, "Average Bitrate Control should be 0x07");

    TEST_PASS();
}

/**
 * Test backward compatibility with MJPEG in isochronous implementation
 */
int test_iso_mjpeg_compatibility()
{
    const uint8_t *ss_desc = CyFxUSBSSConfigDscr;

    // Search for MJPEG format descriptor (subtype 0x06)
    int desc_offset = 9;
    int found_mjpeg_format = 0;

    while (desc_offset < 0xE0) {
        uint8_t desc_len = ss_desc[desc_offset];
        if (desc_len == 0) break;

        uint8_t desc_type = ss_desc[desc_offset + 1];
        uint8_t desc_subtype = ss_desc[desc_offset + 2];

        // Look for class-specific VS interface descriptor with MJPEG format subtype
        if (desc_type == 0x24 && desc_subtype == 0x06) {
            found_mjpeg_format = 1;

            // Verify MJPEG format descriptor structure
            TEST_ASSERT(desc_len == 0x0B, "MJPEG format descriptor should be 11 bytes");

            uint8_t format_index = ss_desc[desc_offset + 3];
            uint8_t num_frame_desc = ss_desc[desc_offset + 4];

            TEST_ASSERT(format_index == 0x01, "MJPEG format index should be 1");
            TEST_ASSERT(num_frame_desc >= 0x01, "Should have at least 1 MJPEG frame descriptor");

            break;
        }

        desc_offset += desc_len;
    }

    TEST_ASSERT(found_mjpeg_format, "MJPEG format descriptor should be preserved in isochronous implementation");
    TEST_PASS();
}

/**
 * Main test runner for isochronous implementation
 */
int main(void)
{
    printf("UVC 1.5 Isochronous Implementation Unit Tests\n");
    printf("=============================================\n\n");

    // Run all tests
    RUN_TEST(test_iso_uvc_version);
    RUN_TEST(test_iso_endpoint_descriptor);
    RUN_TEST(test_iso_ss_endpoint_companion);
    RUN_TEST(test_iso_descriptor_sizes);
    RUN_TEST(test_iso_encoding_unit);
    RUN_TEST(test_iso_uvc15_constants);
    RUN_TEST(test_iso_mjpeg_compatibility);

    // Print results
    printf("\n=============================================\n");
    printf("Isochronous Test Results: %d/%d passed\n", tests_passed, tests_total);

    if (tests_passed == tests_total) {
        printf("All isochronous tests PASSED! ✓\n");
        printf("UVC 1.5 isochronous implementation is working correctly.\n");
        return 0;
    } else {
        printf("Some isochronous tests FAILED! ✗\n");
        printf("Please check the UVC 1.5 isochronous implementation.\n");
        return 1;
    }
}